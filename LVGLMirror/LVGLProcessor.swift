...
enum LVGLProcessor {
    ...
    static func backingStore(of w: Int, h: Int) -> NSBitmapImageRep? {
        ...
        if let rep, let data = rep.bitmapData {
            memset(data, 0, h * rep.bytesPerRow)
        }
        return rep
    }
    
    static func process(jsonPayload: Data, backingStore: NSBitmapImageRep) {
        guard let update = try? JSONDecoder().decode(LVGLUpdate.self, from: jsonPayload), let b64 = update.b64, let rawImgData = Data(base64Encoded: b64) else { return }

        let width  = update.x2 - update.x1 + 1
        let height = update.y2 - update.y1 + 1

        guard width > 0, height > 0, let destBasePtr = backingStore.bitmapData else { return }
        ...
        vImageConvert_RGB565toRGB888(&srcBuffer, &destBuffer, vImage_Flags(kvImageNoFlags))
    }
 }
